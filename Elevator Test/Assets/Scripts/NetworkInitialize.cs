...
namespace Photon
{
	public class NetworkInitialize : Bolt.GlobalEventListener
	{
		...
		struct SimpleTimer
		{
			readonly Single expire;

			public SimpleTimer(Single wait)
			{
				expire = Time.realtimeSinceStartup + wait;
			}
		...
		static SimpleTimer _connectionTimer;
...
//				BoltNetwork.LoadScene ("Elevator");
		}

		void Awake()
		{
			_connectionTimer = new SimpleTimer(0);
		...
		static void ShowSessionList(Dictionary<Guid, UdpSession> sessionList)
		{
			foreach (var session in sessionList)
			{
				UdpSession udpSession = session.Value as UdpSession;
				...
				if (GUILayout.Button(udpSession.HostName, GUILayout.ExpandWidth(true)))
				{
					ServerConnectToken connectToken = new ServerConnectToken
					{
						data = "ConnectTokenData"
				...
				{
					//BoltLog.Info("UdpSession is not a PhotonSession");
					continue;
				}
